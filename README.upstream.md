...
QQ群（本科组），群号：1083408416。

QQ群（高职组），群号：1081066428。
...
- 任务：完成 3 个螺母装配，将螺母套入前方对应的立柱。
- 满分：10 分（完成性 6 分+ 工作效率 4 分）


Task 2: 湾区文创产品精细摆盘

- 任务：将桌面上散落的 1 个大茶杯和 4 个小茶杯摆放到茶盘上。大茶杯摆放在茶盘圆形镂空区域左侧，小茶杯摆放到茶盘圆形镂空区域内。
- 满分：10 分（完成性 7 分+ 工作效率 3 分）


Task 3: 跨境物流小件智能分拣

- 任务：完成不同包装标签的快递件分类。2个有中国标签的放入红色分拣筐，另外2个非中国标签的放入蓝色分拣筐。
- 满分：10 分（完成性 2 分+ 成功性 6 分+ 工作效率 2 分）
...
- Isaac Lab：基于 Isaac Sim 的强化学习框架（本指南以 2.1.0 为例）
- Python 3.10
- CUDA 11.8（与 Isaac Sim 版本匹配）
- LeRobot：用于模型训练与推理服务端

注意：由于ubuntu系统特殊，请尽可能使用和官方软件完全相同的版本，否则提交的作品有极大概率出现兼容性报错的问题。
------
...
进入官网后，首先确认左上角为 4.5.0 版本，若不是，可以点击倒三角修改。

确认版本为 4.5.0 后，点击 "Isaac Sim" 软件右侧的 "Linux" 处，会开始下载 Isaac Sim 压缩包，会下载到你设置的下载路径。
...
- 注意：如果有报错安装失败，提示no module，可以改用下面两条指令安装，先把setup-tools降级到69版本，再安装isaaclab 4.5的依赖flatdict
...
```bash
git clone -b dobot-challenge https://gitee.com/dobot-opensource/Guangdong_Collegiate_Computing_Competition
切换到赛事分支
...
（项目文件 gitee 网址：[点此进入](https://gitee.com/dobot-opensource/Guangdong_Collegiate_Computing_Competition)）

*注意：资产文件较大故没有放置在gitee中，若按上述操作从gitee中拉取，请从赛事官方群中下载assets文件,解压并替换现有的assets文件夹。*
...
运行指令，启动遥操作功能，出现 Isaac Sim 界面，同时 Leader 会通电启动。
...
指令说明：要采集不同任务的数据只需要改变指令中 ` --task=task1 \` 中的1/2/3即可，task1 为3C 电子装配、task2为湾区文创产品精细摆盘，task3 为跨境物流小件智能分拣。

> 注意：各任务的评分判定可视化（有色判定区域框和黄色判定点球体）默认关闭，避免干扰操作。如需可视化的查看判定区域和判定点，可在指令中添加 `--enable_visualization` 参数。判定可视化仅供测试使用，请勿带可视化参数进行训练，最终评分需要关闭判定可视化进行。
...
- 修改 `task` 为使用该数据的任务的一句话描述（策略语言指令）：
  ...
  task1描述："Slide the red nut at the front of the upright onto the corresponding upright."

  task2描述："Place small teacups in the circular hollow of the tea tray, and large teacups on its left."

  task3描述："Put items with Chinese labels into the red basket, and others into the blue basket."
...
**若尚未安装 LeRobot 环境**：建议单独创建 conda 环境以避免与 LeIsaac 依赖冲突。可参考 [LeRobot 官方文档](https://github.com/huggingface/lerobot) 安装。安装后在该环境中执行下面的训练与推理服务端命令。
...
python scripts/evaluation/policy_inference.py \
    ...
    --policy_language_instruction="Slide the red nut at the front of the upright onto the corresponding upright." \
    --device=cuda \
    --enable_cameras \
...
- 修改 `--policy_language_instruction` 为任务的说明（策略语言指令）。
- 修改 `--policy_checkpoint_path` 为前面训练文件的输出路径。
- 此处的评分参数 `eval_rounds=10` `episode_length_s=60` `score_t0_s=60` 均为参考值，官方评分参数请参考下方对应表格。 
> 注意：各任务的评分判定可视化（有色判定区域框和黄色判定点球体）默认关闭，避免干扰操作。如需可视化的查看判定区域和判定点，可添加 `--enable_visualization` 参数。仅供测试使用，请勿带可视化参数进行推理验证，最终评分关闭判定可视化进行。
...
由于虚拟环境与现实时间不同，实际运行时长和t0均为参数值的三倍。如--episode_length_s = 60 时，实际运行时长为 60 * 3 =180s

#### 上述指令中的评分参数仅供参考，下面的官方评分后续会给出具体评定参数，请在自主评分时修改对应任务的参数：

| task1参数                 | 说明                    | 评分参数 |
| -------------------- | ----------------------- | ------ |
| `--eval_rounds`      | 评分回合数              | 10     |
| `--episode_length_s` | 单回合最大时长（秒）    | 未定     |
| `--score_t0_s`       | 效率分基准时间 t0（秒） | 未定     |
...
```
[Score][Task3] Episode 1 | 完成性 2.00 (6/6), 成功性 6.00 (6/6), 工作效率 1.83 (耗时 5.0s / t0 ), 总分 9.83
```

最终平均分输出示例：

```
[Score][Task3] 平均分 | 完成性 1.95, 成功率 5.80, 效率 1.72, 总分 9.47
```

### 6. 提交与打包说明

最终提交物为 **Docker 镜像**，通过百度网盘和赛事邮箱上传作品。测评时主办方在本地运行评分脚本 `policy_scoring.py`，通过端口连接到你镜像内运行的推理服务进行打分。请严格按以下**命名与路径规范**打包，便于自动化测评。

#### 6.1 队伍编号与命名规范

- **队伍编号**：每支队伍有一个 **四位数或五位数的队伍编号**（如 `9001`、`11001`）。
- **作品命名格式**：

| 项目 | 格式 | 示例 |
|------|------|------------------------------|
| Docker 镜像名 | `dobot_{队伍编号}_submission` | `dobot_0001_submission` |
| 镜像 Tag | 按提交次数用 **`v1`、`v2`** 递增，便于区分多次提交；测评时以主办方通知为准 | `v1` |
| 镜像内模型路径 | `/workspace/checkpoints/pretrained_model` | 同上，不可更改 |

- **模型路径**：为统一测评脚本参数，镜像内模型必须且只能放在 `/workspace/checkpoints/pretrained_model`。测评时一律使用该路径作为 `--policy_checkpoint_path`，请勿使用其他路径。
- 若你需上传说明文件，请命名为：`{队伍编号}_readme.txt`（如 `0001_readme.txt`）。

#### 6.2 提交要求概述

- 镜像内需包含：**推理服务端**（与 LeRobot 异步推理接口兼容）、**模型权重与依赖**。
- 容器启动后必须 **监听 5555 端口**，且服务需在 **0.0.0.0** 上监听，以便宿主机能连接。
- 测评时主办方会：从官网获取你的镜像 → 启动容器→ 在宿主机运行 `policy_scoring.py`，使用 `--policy_host=localhost --policy_port=5555` 和 **固定路径** `--policy_checkpoint_path=/workspace/checkpoints/pretrained_model` 连接你的服务并评分。

#### 6.3 接口兼容性

评分脚本通过 gRPC 与推理服务通信，协议与 LeRobot 的 `lerobot.async_inference.policy_server` 一致。你的镜像内可以：

- 直接使用 LeRobot 自带的推理服务端，在容器内放置好权重；客户端连接时会通过 `SendPolicyInstructions` 下发 `pretrained_name_or_path`，服务端在容器内从该路径加载模型。
- 或自行实现与 LeRobot 相同 gRPC 接口的服务（`Ready`、`SendPolicyInstructions`、`SendObservations`、`GetActions` 等）。

客户端会向服务端发送 `PolicySetup`（包含 `policy_type`、`pretrained_name_or_path`、`actions_per_chunk`、观测/动作特征等）。

#### 6.4 镜像内约定：模型路径与启动方式

为便于测评，必须在镜像内使用固定模型路径（见 6.1）：

- **固定的模型目录**：`/workspace/checkpoints/pretrained_model`。权重文件须完整放置于该目录下，与 LeRobot ACT 等格式兼容。
...
若使用 LeRobot 官方服务端，示例启动命令（在服务端内）：
...
# 镜像名须为 dobot_{队伍编号}_submission，模型路径须为 /workspace/checkpoints/pretrained_model
...
请根据你是否使用 GPU 决定是否加 `--gpus all`。

#### 6.6 自测方法

在提交前，建议在本地完整执行“容器内推理服务 + 客户端评分脚本”的流程（自测时镜像名、路径须与 6.1 规范一致）：
...
2. 在宿主机打开另一个终端，进入已配置好的 LeIsaac / x-trainer 环境，运行评分脚本并指向容器端口（以 task1 为例）。模型路径固定为 `/workspace/checkpoints/pretrained_model`：
   ```bash
   cd ~/dobot/x-trainer/
   conda activate leisaac
   source ~/dobot/isaacsim/setup_conda_env.sh
...
3. 若能在宿主机看到评分输出且无连接/超时错误，则说明镜像与接口兼容，可按 6.1 命名规范打包并通过下方的官方提交方式提交。若有报错，请检查：镜像名是否为 `dobot_{编号}_submission`、容器是否暴露 5555、服务是否监听 0.0.0.0、镜像内 `/workspace/checkpoints/pretrained_model` 下是否有完整权重文件。

#### 6.7 提交方式

- **队伍编号**：每支队伍有一个 **四位数或五位数的队伍编号**（如 `9001`、`11001`）。
- **提交通道**：将打包后的镜像文件上传到百度网盘，并将网盘的分享链接用邮件发送到作品收集邮箱`challenge@dobot-robots.com`，邮件**主题**要规范填写"本科/高职组+队伍编号+赛题号+第X次提交"，邮件**正文**包括网盘分享链接、使用的模型类型、镜像（服务端）启动指令、客户端启动指令以及其他的补充说明。
一个邮件只允许发送一个作品，多个作品请用多个邮件分开发送，一个队伍的同一个赛题最多允许提交三次作品，取得分最高的作品计入最终成绩。若该次提交的作品有报错无法运行的情况，不计入提交次数。
...
如果你在参赛过程遇到任何问题，请加入赛事QQ群咨询：1083408416（本科组），1081066428（高职组），申请加群请备注赛项名和学校名。

或者邮件联系我们 [challenge@dobot-robots.com](mailto:challenge@dobot-robots.com)
